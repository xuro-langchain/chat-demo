...
class GuardrailsMiddleware(AgentMiddleware[GuardrailsState]):
    ...
    def __init__(self, model: str = "anthropic:claude-haiku-4-5", block_off_topic: bool = True):
        super().__init__()
        # Use init_chat_model to automatically pick the right provider based on model ID
        # Supports anthropic:, xai:, openai: prefixes
    ...
    def _get_last_message_content(self, state: GuardrailsState) -> str | None:
    ...
    async def _generate_rejection_message(self, content: str) -> AIMessage:
        ...
        try:
            # This rejection message SHOULD stream to the user - it's the actual response
    ...
    async def _classify_query(self, messages: list) -> str | None:
        ...
        """
        # Include more context (up to 6 messages) to better understand follow-ups
